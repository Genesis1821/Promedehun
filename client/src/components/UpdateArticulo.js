...
const UpdateArticulo = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        const comprobacion = values.descripcion === '' || inputCodigoAndResponse.codigo === '';
        if (comprobacion) {
            setInputCodigoAndResponse({ ...inputCodigoAndResponse, response: 'Asegurate de haber llenado los campos' });
        } else {
        ...
        setTimeout(() => {
            setInputCodigoAndResponse({ ...inputCodigoAndResponse, response: '' });
            if (!comprobacion) {
                reset(initalState);
                getAllArticulos();
            }
    ...
    return (
        <form className='compUpdateItem' onSubmit={handleSubmit}>
            ...
            <InputUpdateItem
                ...
                onChangeFunc={handleInputChange}
            />
            {inputCodigoAndResponse.response.length > 0 && <p style={{ margin: '8px 0' }} >{inputCodigoAndResponse.response}</p>}
